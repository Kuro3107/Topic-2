...
import { Layout, Menu, Card, Table, Button, Row, Col, Modal, Select, DatePicker } from "antd";
...
const Delivery = () => {
  ...
  const fetchFarmsAndVarieties = async () => {
    try {
      ...
      const varietiesResponse = await fetch('http://localhost:8080/api/koi-varieties');
  ...
  const fetchData = async () => {
    try {
      ...
      if (poResponse.ok && bookingResponse.ok) {
        const deliveringPOs = poData.filter(po => po.status === 'delivering' || po.status === 'delivered');
  ...
  const handleViewPODetails = async (poId) => {
    try {
      const response = await fetch(`http://localhost:8080/api/podetails/po/${poId}`);
  ...
  const handleUpdate = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/pos/${selectedPO.poId}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        const updatedPOs = poData.map(po => (po.poId === selectedPO.poId ? updatedPO : po));
  ...
  const columns = [
    ...
    { title: "Status", dataIndex: "status", key: "status" },
    {
      ...
      render: (_, record) => (
        <>
          <Button onClick={() => handleViewPODetails(record.poId)}>Xem PODetails</Button>
  ...
  return (
    <Layout>
      ...
      <Modal title="Chi Tiết PODetails" visible={isPODetailsVisible} onCancel={handleClosePODetails}>
        <Table
          ...
          columns={[
            ...
            { title: 'Remaining Price', dataIndex: 'remainingPrice', key: 'remainingPrice' },
            { title: 'Total Koi Price', dataIndex: 'totalKoiPrice', key: 'totalKoiPrice' },
